...
# Move to 5 cm above ground (z in AirSim is negative)
target_z = -0.05
client.moveToZAsync(target_z, 1).join()

print(f"Reached {abs(target_z)*100:.1f} cm height")
...
move_duration = 10  # seconds

try:
    while control_enabled:
        start_time = time.time()

        # Get camera feed and detect objects
